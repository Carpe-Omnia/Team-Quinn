...
function edit_form(name, address, status, sign_id){
  return(
    `<form>
      Name:  <input type="text" value="${name}" id="sign_${sign_id}_name"/>
      Address:<input type="text" value="${address}" id="sign_${sign_id}_address"/>
      <select id="sign_${sign_id}_status" >
        <option value="authorized">Authorized</option>
        <option value="delivered">Delivered</option>
        <option value="picked up">Picked up</option>
      </select>
      <span id="send_sign_${sign_id}_updates">update</span>
    </form`
  )
}
...
const InfoWindow = (props) => {
  ...
  function make_update_form(){
    ...
    document.getElementById(`send_sign_${props.sign_id}_updates`).addEventListener('click', function(){
          ...
          <Card className="card">
            ...
            <CardActions>
              <Button size="small" color="secondary" id={`edit_button_for_sign_${props.sign_id}`}>
                <span onClick={event => make_update_form(event)}> Edit </span>
              </Button>
